...
class GINIComponentAPICoordinatorTests: XCTestCase {
    ...
    func testInitializationWhenImageImported() {
        ...
        componentAPICoordinator = ComponentAPICoordinator(document: document, configuration: GiniConfiguration(), documentService: self.documentService)
        componentAPICoordinator?.start()
        
        XCTAssertNil(componentAPICoordinator?.analysisScreen, "analysis screen should be nil when a image is imported")
        XCTAssertNotNil(componentAPICoordinator?.reviewScreen, "review screen should not be nil when a image is imported")
        XCTAssertNil(componentAPICoordinator?.cameraScreen, "camera screen should be nil when a image is imported")
        
        XCTAssertEqual(componentAPICoordinator?.reviewScreen?.navigationItem.leftBarButtonItem?.title, "Schließen")
    ...
    func testInitializationWhenPDFImported() {
        ...
        componentAPICoordinator = ComponentAPICoordinator(document: pdfDocument, configuration: GiniConfiguration(), documentService: self.documentService)
        componentAPICoordinator?.start()
        
        XCTAssertNotNil(componentAPICoordinator?.analysisScreen, "analysis screen should not be nil when a pdf is imported")
        XCTAssertNil(componentAPICoordinator?.reviewScreen, "review screen should be nil when a pdf is imported")
        XCTAssertNil(componentAPICoordinator?.cameraScreen, "camera screen should be nil when a pdfpdf is imported")
        
        XCTAssertEqual(componentAPICoordinator?.analysisScreen?.navigationItem.leftBarButtonItem?.title, "Schließen")
    }
